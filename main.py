...
def download_image(url : str,number : int,path : str) -> None:
	image_info = requests.get(url)
	image_type_search = re.search(r'\.([^.]+)$',url)
	image_type = ''
	if image_type_search:
		image_type = image_type_search.group(1)
	full_name = f'{str(number)}.{image_type}'
	with open(f'./{title}/{full_name}','wb') as f:
...
if __name__ == '__main__':
	...
	all_page = int(page_find_all[len(page_find_all) - 2])
	...
	for i in range(all_page):
		...
		for url in urls:
			count += 1
			print(f'第{count}个图片开始下载。')
			html = session.get(url)
			html_text = html.text
			image_url = re.search(r'<img id="img" src="(.*?)".*?>',html_text,re.S)
			download_image(image_url.group(1),count,title)
